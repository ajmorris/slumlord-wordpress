...
class UsersController < ApplicationController
  ...
  def ip
    ...
    if params[:ip].blank?
      if !@user.secure_ip.nil?
        ...
        flash[:success] = "Your WordPress server is now accessible by anyone."
      end
    else
      if !check_ip(params[:ip])
        ...
        @user.save(validate: false)
        flash[:success] = "Your WordPress server will now work only with requests from IP #{@user.secure_ip}."
      end
    end
    redirect_to me_path
  end
  ...
  def create_wordpress_instance
    ...
    @user.apache = i[0]["NetworkSettings"]["PortMapping"]["80"]
